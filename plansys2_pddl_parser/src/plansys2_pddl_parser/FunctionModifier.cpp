...
plansys2_msgs::NodeSharedPtr FunctionModifier::getTree( plansys2_msgs::Tree & tree, const Domain & d, const std::vector<std::string> & replace ) const {
    plansys2_msgs::NodeSharedPtr node = std::make_shared<plansys2_msgs::Node>();
    node->node_type = plansys2_msgs::Node::FUNCTION_MODIFIER;
    node->modifier_type = getFunModType(name);
    node->node_id = tree.nodes.size();
    tree.nodes.push_back(*node);

    if (modifiedGround) {
        plansys2_msgs::NodeSharedPtr child = modifiedGround->getTree(tree, d, replace);
    ...
    plansys2_msgs::NodeSharedPtr child = modifierExpr->getTree(tree, d, replace);
    tree.nodes[node->node_id].children.push_back(child->node_id);
