...
void Action::PDDLPrint( std::ostream & s,
			unsigned indent,
...
plansys2_msgs::NodeSharedPtr Action::getTree( plansys2_msgs::Tree & tree,
						const Domain & d,
						const std::vector<std::string> & replace ) const {

  plansys2_msgs::NodeSharedPtr node = std::make_shared<plansys2_msgs::Node>();
  node->node_type = plansys2_msgs::Node::ACTION;
  node->node_id = tree.nodes.size();
  tree.nodes.push_back(*node);

    if (pre) {
      plansys2_msgs::NodeSharedPtr child = pre->getTree(tree, d, replace);
      tree.nodes[node->node_id].children.push_back(child->node_id);
    }
    
    if (eff) {
      plansys2_msgs::NodeSharedPtr child = eff->getTree(tree, d, replace);
      tree.nodes[node->node_id].children.push_back(child->node_id);
    }
...
void Action::parseConditions( Stringreader & f,
